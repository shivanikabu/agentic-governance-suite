...
========================================
A Streamlit-based tool that automatically generates comprehensive documentation
for GitHub repositories using Repomix and LLM analysis.

Documentation Types Generated:
- API Documentation (endpoints, request/response formats, examples)
- Developer Guide (setup, structure, workflow, testing)
- Architecture Documentation (overview, interactions, design decisions)

The tool can also perform:
- Code Review (architecture, security, performance analysis)
- Test Coverage Analysis (untested components, test quality)

Features:
- GitHub repository integration via Repomix
- LLM-powered documentation generation using Azure OpenAI GPT-4o
...
def run_repomix(github_url):
    ...
    Repomix converts an entire GitHub repository into a single XML file
    containing all source code, making it suitable for LLM analysis.
    ...
    Output:
        Creates 'repomix-output.xml' in the current directory
    ...
    Note:
        The repomix_cli path should be adjusted based on your system:
        - Linux: /home/username/.nvm/versions/node/vX.X.X/bin/repomix
    ...
    try:
        ...
        result = subprocess.run([
            ...
            repomix_cli,         # Repomix CLI script
...
def read_xml_file(file_path):
    ...
    Note:
        The XML file contains the entire codebase structure with file contents,
...
def get_prompt(task_type):
    ...
    Args:
        task_type (str): Type of analysis to perform
            ...
            - "Test Coverage": Testing and coverage analysis  
    ...
    Note:
        Each prompt is carefully structured to guide the LLM to produce
        comprehensive, actionable analysis with specific file references
    """
...
def analyze_with_llm(xml_content, task_type):
    ...
    # GPT-4o provides superior code analysis compared to earlier models
...
# Options: "Code Review", "Test Coverage", "Documentation Generation"
    ...
    if not github_url:
        ...
        if success:
            ...
            if xml_content:
                ...
                with st.spinner("Generating documentation with LLM..."):
